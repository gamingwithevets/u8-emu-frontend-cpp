...
dlabels::dlabels(class MCU *mcu) {
	...
	case HW_ES_PLUS:
		...
		ramlabels.push_back({0x60, 24, "Operator stack", "Stores operator IDs to be parsed.\n"
				"The maximum operator count is 24, however in MathIO there are some hard-coded limits for MathIO functions such as roots, fractions, etc.\n"
				"Exponentiation is always limited to 10 operators."});
		ramlabels.push_back({0x78, 100, "Number stack", "Stores numbers to perform calculations on.\nIn most cases, a midway result will be stored here as well.\nThe maximum number stack length is 10."});
		...
		ramlabels.push_back({0x100, 1, "Result format", "High nibble: Result display format.\nLow nibble: Variable store format.\n"
			"0 None  1 Sexagesimal (degs mins secs)\n"
		...
		if (mcu->config->hardware_id == HW_ES_PLUS) {
			ramlabels.push_back({offset+930, 400, "Multipurpose", "In MathI input, this area stores bounding boxes(?) used to draw MathI graphics (not used by square roots).\n\nThe math library also uses this area to store temporary numbers."});
			offset += 424;
		}
