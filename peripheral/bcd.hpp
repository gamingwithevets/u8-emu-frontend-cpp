...
// REG
// BCD SFR addresses
const int REG_BCDCMD = 0x400; // BCD command register
const int REG_BCDCON = 0x402; // BCD control register
const int REG_BCDMCN = 0x404; // BCD macro control register
const int REG_BCDMCR = 0x405; // BCD macro register
const int REG_BCDFLG = 0x410; // BCD flag register
const int REG_BCDLLZ = 0x414; // BCD low-level zero(?) register
const int REG_BCDMLZ = 0x415; // BCD mid-level zero(?) register
...
const int STATE_DIV_0 = 0x18;
const int STATE_DIV_1 = 0x19;
const int STATE_DIV_2 = 0x1a;
const int STATE_DIV_3 = 2;
const int STATE_DIV_4 = 1;
const int STATE_DIV_5 = 0;
const int STATE_DIV_6 = 0x1b;
...
private:
	...
	void calc_sl(bool ex)
	{
		...
		case 0: {
			...
			b3 = ex ? mcu->sfr[RegAdr(RegPrev(dst), 11)] : 0;
			b = (byte)((b2 << 4) | (b3 >> 4));
		...
		case 1: {
			...
			b = ex ? mcu->sfr[RegAdr(RegPrev(dst), 11)] : 0;
			mcu->sfr[RegAdr(dst, 0)] = b;
			break;
		}
		case 2: {
			...
			for (int i = 0; i < 2; i++) {
                b = ex ? mcu->sfr[RegAdr(RegPrev(dst), 10 + i)] : 0;
		...
		case 3: {
			...
			for (int i = 0; i < 4; i++) {
                b = ex ? mcu->sfr[RegAdr(RegPrev(dst), 8 + i)] : 0;
				mcu->sfr[RegAdr(dst, i)] = b;
			}
			break;
		}
	...
	void exec_Add_Sub()
	{
		...
		if (calc_en && calc_pos == 0)
		{
			uint num2 = 1u;
			uint num3 = 0u;
			bool flag = calc_mode == CAL_ADD || calc_mode == CAL_SUB;
			for (int i = 0; i < calc_len; i += 2)
			{
				...
				if (flag) mcu->sfr[RegAdr(dst, i * 2) + 1] = num >> 8 & 0xff; mcu->sfr[RegAdr(dst, i * 2)] = num & 0xff;
#ifdef BCDDEBUG
				...
				if (flag) {
					if (i + 1 == calc_len) num = 0u;
					mcu->sfr[RegAdr(dst, i * 2 + 2) + 1] = num >> 8 & 0xff; mcu->sfr[RegAdr(dst, i * 2 + 2)] = num & 0xff;
				}
			}
		}
		if ((calc_mode == CAL_ADD || calc_mode == CAL_SUB) && (calc_en_d || calc_en_dd))
		{
			calc_pos += 2;
	...
	void exec_Sft_Con_Cp() {
		byte b = 0;
		switch (calc_mode & (calc_en ? 0xf : 0)) {
		case CAL_SL:
			calc_sl(false);
			break;
		case CAL_SR:
	...
	void update_LLZ_MLZ()
	{
		...
		for (int num = 11; num >= 0; num--) {
			...
			if (num < calc_len * 2 && (b & 0xF0u) != 0) break;
			b2++;
			if (num < calc_len * 2 && (b & 0xFu) != 0) break;
			b2++;
		}
		byte b3 = 0;
	...
	void state_manage() {
		byte b = 0;
#ifdef BCDDEBUG
        printf("state_manage: mul_mode = %d, div_mode = %d, divsn_mode = %d, sft_mode = %d\n", mul_mode, div_mode, divsn_mode, sft_mode);
#endif // BCDDEBUG
		...
		b = mcu->sfr[REG_BCDMCR];
		mcu->sfr[REG_BCDMCR] = (b & 0x7f) | ((mul_mode | div_mode | divsn_mode | sft_mode) ? 0x80 : 0);
	}
	...
	void state_manage_init() {
		byte b = 0;
		if (mul_mode) state_set(13, 0, 0, (uint)(32 + (mcu->sfr[RegAdr(0, 0)] & 0xf)));
		else if (div_mode) state_set(8, 0, 1, 24);
		else if (divsn_mode) state_set(12, 0, 1, 24);
		else switch ((BMC >> 1) & 0xF) {
        case 1:
            if ((BMC & (true ? 1u : 0u)) != 0) state_set(13, 0, 0, (uint)(32 + (mcu->sfr[RegAdr(0, 0)] & 0xf)));
            else state_set(11, 1, 3, 24u);
            mul_mode = true;
            break;
        case 2:
            if ((BMC & (true ? 1u : 0u)) != 0) state_set(8, 0, 1, 24u);
            else state_set(11, 1, 3, 16u);
            div_mode = true;
            break;
        case 3:
            if ((BMC & (true ? 1u : 0u)) != 0) state_set(12, 0, 1, 24u);
            else state_set(11, 1, 3, 32u);
            divsn_mode = true;
            break;
        case 4:
        case 5:
...
#ifdef BCDDEBUG
            printf("=== loop ===\ncalc_en = %d, macro_state = %02x, calc_len = %d, calc_pos = %d, repeat = %d\n", calc_en, macro_state, calc_len, calc_pos, repeat);
#endif // BCDDEBUG
            state_manage();
            calc_en_dd = calc_en_d;
            calc_en_d = calc_en;
#ifdef BCDDEBUG
            printf("setup state:\ncalc_mode = %d, src = %d, dst = %d, macro_state = %02x\ncalc_en = %d, calc_len = %d, calc_pos = %d\n", calc_mode, src, dst, macro_state, calc_en, calc_len, calc_pos);
#endif // BCDDEBUG
            exec_calc();
#ifdef BCDDEBUG
            printf("=== end loop ===\n");
...
uint8_t bcdcmd_mcr(mcu *mcu, uint16_t addr, uint8_t val) {
    mcu->sfr[addr] = val;
#ifdef BCDDEBUG
    printf("===== START =====\nBCDCMD = %02x, BCDMCR = %02x, write: BCD%s - CSR:PC = %X:%04XH\n", mcu->sfr[REG_BCDCMD], mcu->sfr[REG_BCDMCR], addr == 0x400 ? "CMD" : "MCR", mcu->core->regs.csr, mcu->core->regs.pc);
#endif // BCDDEBUG
    mcu->bcd->perApi_Run();
#ifdef BCDDEBUG
    printf("===== END =====\n");
